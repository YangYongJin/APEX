    ...
    def __init__(
        ...
        if current_trainer == "IVLP":
            self.resblocks = nn.ModuleList([
                ResidualAttentionBlock_IVLP(width, heads, True, text_layer, i, design_details, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer) if prompts_needed > i else ResidualAttentionBlock_IVLP(width, heads, False, text_layer, i, design_details, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        prompt_till_layer_vision = design_details['vision_depth']
        self.transformer = Transformer(
            width,
            layers,
            heads,
            prompts_needed=prompt_till_layer_vision,
            text_layer=False,
...
class TextTransformer(nn.Module):
    ...
    def forward_transformer(self, x, text):
        cast_dtype = self.transformer.get_cast_dtype()
        seq_len = x.shape[1]
        ...
        if self.cls_emb is not None:
            ...
            cls_mask = self.build_cls_mask(text, cast_dtype)
            if attn_mask is not None:
                attn_mask = attn_mask[None, :seq_len, :seq_len] + cls_mask[:, :seq_len, :seq_len].to(attn_mask.device)
